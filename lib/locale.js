...
var defaultNumbers = { // en-US standard
  grouping: ',',
  decimal: '.'
};
var defaultLocaleNameTemplate = template('<%= language %> (<%= country %>)');
var defaultFormatDateTemplate = template('<%= M %>/<%= D %>/<%= YY %>'); // en-US standard
...
function locale(Language, Country, name) {
  ...
  Locale.prototype.formatNumber = function (number, options) {
    ...
    var numbers = bundle.numbers[this] || bundle.numbers[this.language] || defaultNumbers;
  ...
  Locale.prototype.formatDate = function (date, pattern) {
    ...
    var tmpl = pattern ? template(dates[maybe(Str)(pattern)] || pattern) : defaultFormatDateTemplate;
    return tmpl(splitDate(t.Dat(date), dates));
  };
